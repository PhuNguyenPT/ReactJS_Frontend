// VietnameseSubject with translation keys
export type VietnameseSubjectTranslationKey =
  | "subjects.bieu_dien_nghe_thuat"
  | "subjects.chi_huy_tai_cho"
  | "subjects.chung_chi_quy_doi_tieng_anh"
  | "subjects.chuyen_mon_am_nhac"
  | "subjects.chuyen_mon_am_nhac_1"
  | "subjects.chuyen_mon_am_nhac_2"
  | "subjects.cong_nghe"
  | "subjects.cong_nghe_cong_nghiep"
  | "subjects.cong_nghe_nong_nghiep"
  | "subjects.dia_ly"
  | "subjects.doc_dien_cam"
  | "subjects.doc_hieu"
  | "subjects.gdktpl"
  | "subjects.ghi_am_xuong_am"
  | "subjects.hat"
  | "subjects.hat_bieu_dien_nhac_cu"
  | "subjects.hat_mua"
  | "subjects.hat_xuong_am"
  | "subjects.hoa_hoc"
  | "subjects.hoa_thanh"
  | "subjects.khoa_hoc_tu_nhien"
  | "subjects.khoa_hoc_xa_hoi"
  | "subjects.ky_xuong_am"
  | "subjects.lich_su"
  | "subjects.nang_khieu"
  | "subjects.nang_khieu_1"
  | "subjects.nang_khieu_2"
  | "subjects.nang_khieu_am_nhac_1"
  | "subjects.nang_khieu_am_nhac_2"
  | "subjects.nang_khieu_anh_bao_chi"
  | "subjects.nang_khieu_bao_chi"
  | "subjects.nang_khieu_bieu_dien_nghe_thuat"
  | "subjects.nang_khieu_kien_thuc_van_hoa_xa_hoi_nghe_thuat"
  | "subjects.nang_khieu_mam_non"
  | "subjects.nang_khieu_mam_non_1"
  | "subjects.nang_khieu_mam_non_2"
  | "subjects.nang_khieu_quay_phim_truyen_hinh"
  | "subjects.nang_khieu_skda_1"
  | "subjects.nang_khieu_skda_2"
  | "subjects.nang_khieu_tdtt"
  | "subjects.nang_khieu_thuyet_trinh"
  | "subjects.nang_khieu_ve_1"
  | "subjects.nang_khieu_ve_2"
  | "subjects.ngu_van"
  | "subjects.phat_trien_chu_de_pho_tho"
  | "subjects.sinh_hoc"
  | "subjects.tieng_anh"
  | "subjects.tieng_duc"
  | "subjects.tieng_han"
  | "subjects.tieng_nga"
  | "subjects.tieng_nhat"
  | "subjects.tieng_phap"
  | "subjects.tieng_trung"
  | "subjects.tin_hoc"
  | "subjects.toan"
  | "subjects.tu_duy_giai_quyet_ngu_van_de"
  | "subjects.vat_ly"
  | "subjects.ve_hinh_hoa"
  | "subjects.ve_hinh_hoa_my_thuat"
  | "subjects.ve_my_thuat"
  | "subjects.ve_nang_khieu"
  | "subjects.ve_trang_tri"
  | "subjects.ve_trang_tri_mau"
  | "subjects.xay_dung_kich_ban_su_kien";

export const VietnameseSubject = {
  BIEU_DIEN_NGHE_THUAT: "subjects.bieu_dien_nghe_thuat",
  CHI_HUY_TAI_CHO: "subjects.chi_huy_tai_cho",
  CHUNG_CHI_QUY_DOI_TIENG_ANH: "subjects.chung_chi_quy_doi_tieng_anh",
  CHUYEN_MON_AM_NHAC: "subjects.chuyen_mon_am_nhac",
  CHUYEN_MON_AM_NHAC_1: "subjects.chuyen_mon_am_nhac_1",
  CHUYEN_MON_AM_NHAC_2: "subjects.chuyen_mon_am_nhac_2",
  CONG_NGHE: "subjects.cong_nghe",
  CONG_NGHE_CONG_NGHIEP: "subjects.cong_nghe_cong_nghiep",
  CONG_NGHE_NONG_NGHIEP: "subjects.cong_nghe_nong_nghiep",
  DIA_LY: "subjects.dia_ly",
  DOC_DIEN_CAM: "subjects.doc_dien_cam",
  DOC_HIEU: "subjects.doc_hieu",
  GDKTPL: "subjects.gdktpl",
  GHI_AM_XUONG_AM: "subjects.ghi_am_xuong_am",
  HAT: "subjects.hat",
  HAT_BIEU_DIEN_NHAC_CU: "subjects.hat_bieu_dien_nhac_cu",
  HAT_MUA: "subjects.hat_mua",
  HAT_XUONG_AM: "subjects.hat_xuong_am",
  HOA_HOC: "subjects.hoa_hoc",
  HOA_THANH: "subjects.hoa_thanh",
  KHOA_HOC_TU_NHIEN: "subjects.khoa_hoc_tu_nhien",
  KHOA_HOC_XA_HOI: "subjects.khoa_hoc_xa_hoi",
  KY_XUONG_AM: "subjects.ky_xuong_am",
  LICH_SU: "subjects.lich_su",
  NANG_KHIEU: "subjects.nang_khieu",
  NANG_KHIEU_1: "subjects.nang_khieu_1",
  NANG_KHIEU_2: "subjects.nang_khieu_2",
  NANG_KHIEU_AM_NHAC_1: "subjects.nang_khieu_am_nhac_1",
  NANG_KHIEU_AM_NHAC_2: "subjects.nang_khieu_am_nhac_2",
  NANG_KHIEU_ANH_BAO_CHI: "subjects.nang_khieu_anh_bao_chi",
  NANG_KHIEU_BAO_CHI: "subjects.nang_khieu_bao_chi",
  NANG_KHIEU_BIEU_DIEN_NGHE_THUAT: "subjects.nang_khieu_bieu_dien_nghe_thuat",
  NANG_KHIEU_KIEN_THUC_VAN_HOA_XA_HOI_NGHE_THUAT:
    "subjects.nang_khieu_kien_thuc_van_hoa_xa_hoi_nghe_thuat",
  NANG_KHIEU_MAM_NON: "subjects.nang_khieu_mam_non",
  NANG_KHIEU_MAM_NON_1: "subjects.nang_khieu_mam_non_1",
  NANG_KHIEU_MAM_NON_2: "subjects.nang_khieu_mam_non_2",
  NANG_KHIEU_QUAY_PHIM_TRUYEN_HINH: "subjects.nang_khieu_quay_phim_truyen_hinh",
  NANG_KHIEU_SKDA_1: "subjects.nang_khieu_skda_1",
  NANG_KHIEU_SKDA_2: "subjects.nang_khieu_skda_2",
  NANG_KHIEU_TDTT: "subjects.nang_khieu_tdtt",
  NANG_KHIEU_THUYET_TRINH: "subjects.nang_khieu_thuyet_trinh",
  NANG_KHIEU_VE_1: "subjects.nang_khieu_ve_1",
  NANG_KHIEU_VE_2: "subjects.nang_khieu_ve_2",
  NGU_VAN: "subjects.ngu_van",
  PHAT_TRIEN_CHU_DE_PHO_THO: "subjects.phat_trien_chu_de_pho_tho",
  SINH_HOC: "subjects.sinh_hoc",
  TIENG_ANH: "subjects.tieng_anh",
  TIENG_DUC: "subjects.tieng_duc",
  TIENG_HAN: "subjects.tieng_han",
  TIENG_NGA: "subjects.tieng_nga",
  TIENG_NHAT: "subjects.tieng_nhat",
  TIENG_PHAP: "subjects.tieng_phap",
  TIENG_TRUNG: "subjects.tieng_trung",
  TIN_HOC: "subjects.tin_hoc",
  TOAN: "subjects.toan",
  TU_DUY_GIAI_QUYET_NGU_VAN_DE: "subjects.tu_duy_giai_quyet_ngu_van_de",
  VAT_LY: "subjects.vat_ly",
  VE_HINH_HOA: "subjects.ve_hinh_hoa",
  VE_HINH_HOA_MY_THUAT: "subjects.ve_hinh_hoa_my_thuat",
  VE_MY_THUAT: "subjects.ve_my_thuat",
  VE_NANG_KHIEU: "subjects.ve_nang_khieu",
  VE_TRANG_TRI: "subjects.ve_trang_tri",
  VE_TRANG_TRI_MAU: "subjects.ve_trang_tri_mau",
  XAY_DUNG_KICH_BAN_SU_KIEN: "subjects.xay_dung_kich_ban_su_kien",
} as const;

// Vietnamese mappings for subjects
export const VietnameseSubjectValues: Record<
  VietnameseSubjectTranslationKey,
  string
> = {
  "subjects.bieu_dien_nghe_thuat": "Biểu diễn nghệ thuật",
  "subjects.chi_huy_tai_cho": "Chỉ huy tại chỗ",
  "subjects.chung_chi_quy_doi_tieng_anh":
    "Chứng chỉ tiếng Anh (đã quy đổi điểm)",
  "subjects.chuyen_mon_am_nhac": "Chuyên môn âm nhạc",
  "subjects.chuyen_mon_am_nhac_1": "Chuyên môn âm nhạc 1",
  "subjects.chuyen_mon_am_nhac_2": "Chuyên môn âm nhạc 2",
  "subjects.cong_nghe": "Công Nghệ",
  "subjects.cong_nghe_cong_nghiep": "Công nghệ Công nghiệp",
  "subjects.cong_nghe_nong_nghiep": "Công nghệ Nông nghiệp",
  "subjects.dia_ly": "Địa Lý",
  "subjects.doc_dien_cam": "Đọc diễn cảm",
  "subjects.doc_hieu": "Đọc hiểu",
  "subjects.gdktpl": "Giáo dục Kinh tế và Pháp luật",
  "subjects.ghi_am_xuong_am": "Ghi âm - xướng âm",
  "subjects.hat": "Hát",
  "subjects.hat_bieu_dien_nhac_cu": "Hát hoặc biểu diễn nhạc cụ",
  "subjects.hat_mua": "Hát - Múa",
  "subjects.hat_xuong_am": "Hát xướng âm",
  "subjects.hoa_hoc": "Hóa Học",
  "subjects.hoa_thanh": "Hòa thanh",
  "subjects.khoa_hoc_tu_nhien": "Khoa học tự nhiên",
  "subjects.khoa_hoc_xa_hoi": "Khoa học xã hội",
  "subjects.ky_xuong_am": "Ký xướng âm",
  "subjects.lich_su": "Lịch Sử",
  "subjects.nang_khieu": "Năng khiếu",
  "subjects.nang_khieu_1": "Năng khiếu 1",
  "subjects.nang_khieu_2": "Năng khiếu 2",
  "subjects.nang_khieu_am_nhac_1": "Năng khiếu Âm nhạc 1 (Hát, xướng âm)",
  "subjects.nang_khieu_am_nhac_2": "Năng khiếu Âm nhạc 2 (Thẩm âm, tiết tấu)",
  "subjects.nang_khieu_anh_bao_chi": "Năng khiếu ảnh báo chí",
  "subjects.nang_khieu_bao_chi": "Năng khiếu báo chí",
  "subjects.nang_khieu_bieu_dien_nghe_thuat": "Năng khiếu Biểu diễn nghệ thuật",
  "subjects.nang_khieu_kien_thuc_van_hoa_xa_hoi_nghe_thuat":
    "Năng khiếu Kiến thức văn hóa - xã hội - nghệ thuật",
  "subjects.nang_khieu_mam_non": "Năng khiếu Mầm non",
  "subjects.nang_khieu_mam_non_1":
    "Năng khiếu Mầm non 1 (Kể chuyện, đọc, diễn cảm)",
  "subjects.nang_khieu_mam_non_2": "Năng khiếu Mầm non 2 (Hát)",
  "subjects.nang_khieu_quay_phim_truyen_hinh":
    "Năng khiếu quay phim truyền hình",
  "subjects.nang_khieu_skda_1": "Năng khiếu SKĐA 1",
  "subjects.nang_khieu_skda_2": "Năng khiếu SKĐA 2",
  "subjects.nang_khieu_tdtt": "Năng khiếu TDTT",
  "subjects.nang_khieu_thuyet_trinh": "Năng khiếu thuyết trình",
  "subjects.nang_khieu_ve_1": "Năng khiếu Vẽ Nghệ thuật 1",
  "subjects.nang_khieu_ve_2": "Năng khiếu Vẽ Nghệ thuật 2",
  "subjects.ngu_van": "Ngữ Văn",
  "subjects.phat_trien_chu_de_pho_tho": "Phát triển chủ đề và phổ thơ",
  "subjects.sinh_hoc": "Sinh Học",
  "subjects.tieng_anh": "Tiếng Anh",
  "subjects.tieng_duc": "Tiếng Đức",
  "subjects.tieng_han": "Tiếng Hàn",
  "subjects.tieng_nga": "Tiếng Nga",
  "subjects.tieng_nhat": "Tiếng Nhật",
  "subjects.tieng_phap": "Tiếng Pháp",
  "subjects.tieng_trung": "Tiếng Trung",
  "subjects.tin_hoc": "Tin Học",
  "subjects.toan": "Toán",
  "subjects.tu_duy_giai_quyet_ngu_van_de": "Tư duy Khoa học Giải quyết vấn đề",
  "subjects.vat_ly": "Vật Lý",
  "subjects.ve_hinh_hoa": "Vẽ Hình họa",
  "subjects.ve_hinh_hoa_my_thuat": "Vẽ Hình họa mỹ thuật",
  "subjects.ve_my_thuat": "Vẽ Mỹ thuật",
  "subjects.ve_nang_khieu": "Vẽ Năng Khiếu",
  "subjects.ve_trang_tri": "Vẽ trang trí",
  "subjects.ve_trang_tri_mau": "Vẽ trang trí màu",
  "subjects.xay_dung_kich_ban_su_kien": "Xây dựng kịch bản sự kiện",
} as const;

// Type guard and helper functions for subjects
function isVietnameseSubjectTranslationKey(
  key: string,
): key is VietnameseSubjectTranslationKey {
  return key in VietnameseSubjectValues;
}

export const getVietnameseSubjectValue = (translationKey: string): string => {
  if (isVietnameseSubjectTranslationKey(translationKey)) {
    return VietnameseSubjectValues[translationKey];
  }
  return translationKey;
};

export const getVietnameseSubjectTranslationKey = (
  vietnameseValue: string,
): string => {
  const entry = Object.entries(VietnameseSubjectValues).find(
    ([, value]) => value === vietnameseValue,
  );
  return entry ? entry[0] : vietnameseValue;
};
